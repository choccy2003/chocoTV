    ...
    <div onClick={()=>{
      ...
      window.scrollTo(0, 0);
        ...
        <div style={{display:"flex",flexDirection:"column",maxHeight:"105px"}}>
            <div style={{fontSize:"17px",maxHeight:"67px",marginTop:"2%"}}>{props.data.videoTitle}</div>
            <div style={{fontSize:"14px",marginTop:"auto"}}>{props.data.channelName}</div>
            <div style={{display:"inline-flex",fontSize:"14px",marginTop:"0.5%"}}><div>{props.data.viewCount} views</div><div style={{backgroundColor:"white",width:"4px",borderRadius:"100%",height:"4px",margin:"9px 6px 9px 9px"}}></div><div>{date.toLocaleString('en-US')}</div></div>
        </div>
    </div>
    </>
  )
